...
def read_option():
    ...
    parser.add_argument('--test_only_train', help='test with only data similar to training (missing characteristics);', action="store_true", default=False)
    ...
    parser.add_argument('--pool_size', help='Pool size for proposal;', type=int, default=10)
    parser.add_argument('--top_k', help='Top k for proposal;', type=int, default=5)
    parser.add_argument('--num_outer_loops', help='Number of outer loop in Probabilistic prompts;', type=int, default=5)
    parser.add_argument('--max_text_len', help='Max text len (the larger the more informative and heavier the computation)', type=int, default=40)
    ...
    # FedMSplit
    parser.add_argument('--fedmsplit_prox_lambda', help='FedMSplit proximal lambda;', type=float, default=0.01)
...
def initialize(option):
    ...
    if not option['server_with_cpu']:
        if option['model'] in ["Fed-Prime", "Fed-Intra", "Fed-Inter", "FedProx-P"]:
            model = utils.fmodule.Model(option['pool_size'], option['top_k']).to(utils.fmodule.dev_list[0])
        else:
            model = utils.fmodule.Model().to(utils.fmodule.dev_list[0])

    else:
        if option['model'] in ["Fed-Prime", "Fed-Intra", "Fed-Inter", "FedProx-P"]:
            model = utils.fmodule.Model(option['pool_size'], option['top_k'])
        else:
